...
# awk '{if (NR%4==1) $1=$1 "/1"; print}' ${INPDIR}/POMV24HPIR1Unmapped.out.mate1.fastq > ${INPDIR}/POMV24HPIR1Unmapped.out.mate1.mark12
# awk '{if (NR%4==1) $1=$1 "/2"; print}' ${INPDIR}/POMV24HPIR1Unmapped.out.mate2.fastq > ${INPDIR}/POMV24HPIR1Unmapped.out.mate2.mark12

#Trinity --seqType fq  --SS_lib_type RF --max_memory 40G --min_kmer_cov 1 --CPU 8 \
#--left ${INPDIR}/POMV24HPIR1Unmapped.out.mate1.mark12 \
#--right ${INPDIR}/POMV24HPIR1Unmapped.out.mate2.mark12 \
#--output ${OUTDIR}/Trinity

## To run Trinity, reads have to be formatted in the old Ilumina format, using the /1 and /2 marks. 
## STAR does not have an option to add these marks, however, you can easily add them using:
awk '{if (NR%4==1) $1=$1 "/1"; print}' ${INPDIR}/ISAV24HPIR1Unmapped.out.mate1.fastq > ${INPDIR}/ISAV24HPIR1Unmapped.out.mate1.mark12
awk '{if (NR%4==1) $1=$1 "/2"; print}' ${INPDIR}/ISAV24HPIR1Unmapped.out.mate2.fastq > ${INPDIR}/ISAV24HPIR1Unmapped.out.mate2.mark12

Trinity --seqType fq  --SS_lib_type RF --max_memory 40G --min_kmer_cov 1 --CPU 8 \
--left ${INPDIR}/ISAV24HPIR1Unmapped.out.mate1.mark12 \
--right ${INPDIR}/ISAV24HPIR1Unmapped.out.mate2.mark12 \
--output ${OUTDIR}/Trinity
